export type Response = { mensaje: string; error?: Error }
type resParams = { mensaje: string; error?: string }
...
export default function response(
  { mensaje, error: err }: resParams,
): Response {
  return err ? { mensaje, error: new Error(`${err}`) }
    : { mensaje };
}